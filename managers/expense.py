...
class ExpenseManager:
    ...
    async def rename_expense(cls, payload):
        ...
        updated_expense = await ExpenseRepository.edit_expense(
            cls._expense, {"title": payload.get("title")}
        )
        serialized_updated_expense = await ExpenseSerializer.serialize_expense(
            updated_expense
        )
