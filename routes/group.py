...
async def create_group(request):
    ...
    return response.json(
        {"message": "Group created successfully", "data": group}, status=201
    )
...
async def settle_up(request, static_id):
    params = request.args
    settlements = await GroupManager.settle_up(params)
    return response.json({"message": "Settled up expenses", "settlements": settlements})
